...
class Database {
  ...
  async addTournToUser(tourn, user) {
    ...
    try {
      ...
      await collection.updateOne(query, {
        $push: { registeredToo: res2.start },
      });
      await collection.updateOne(query, {
        $push: { registeredToo: res2.date },
      });
    } catch (err) {
      console.log(err);
    } finally {
  ...
  async updateWinInTourn(tourn, user, wins) {
    const client = await MongoClient.connect(config.DB_URI, {
      useNewUrlParser: true,
    }).catch((err) => {
      console.log(err);
    ...
    try {
      ...
      //console.log(query)
      //let update = { listtoplay:toplay}
      await collection.updateOne(query, { $set: { wins: wins } });
      //do the time verificaiton in servrr file
  ...
  async updateLossInTourn(tourn,user,loss){
...
try {
    ...
    //console.log(query)
    //let update = { listtoplay:toplay}
   await collection.updateOne(query, {$set: {loss: loss}});
        //do the time verificaiton in servrr file
...
async getUSerDataFromTorun(tourn,user){
    const client = await MongoClient.connect(config.DB_URI, { useNewUrlParser: true })
    .catch(err => { console.log(err); });
...
try {
    ...
    let res = await collection.findOne(query);
      ...
      obj.push({
        ...
        losses: res.loss,
        draws: res.draw,
        playnext: res.playnext,
        listtoplay: res.listtoplay,
      });
...
async updateRecord(user,loss,win){
    const client = await MongoClient.connect(config.DB_URI, { useNewUrlParser: true })
    .catch(err => { console.log(err); });
...
try {
   ...
   // console.log(res);
    ...
    var updatewin= res.win + win;

    var updateloss= res.loss + loss


    //console.log(query)
    //let update = { listtoplay:toplay}
   await collection.updateOne(query, {$set: {loss: updateloss}});
   await collection.updateOne(query, {$set: {win: updatewin}});
        //do the time verificaiton in servrr file
